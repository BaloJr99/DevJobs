<form action="" class="md:w-1/2 space-y-5" wire:submit.prevent='editVacancy'>
    ...
    <div class="text-center">
        ...
        <div class="my-5 w-full flex justify-center">
            @if ($image)
                <img src="{{ asset('storage/vacancies/' . $image) }}" alt="image" class="w-2/3">
            @endif
        </div>
        <x-input-label for="image" :value="__('New Image')" />
        <x-text-input id="new_image" class="block mt-1 w-full" type="file" accept="image/*" wire:model="new_image" />
        <div class="my-5 w-full flex justify-center">
            @if ($new_image)
                <img src="{{ $new_image -> temporaryUrl() }}" alt="preImage" class="w-2/3">
            @endif
        </div>
        @error('new_image')
            <livewire:show-alert :message="$message"/>
